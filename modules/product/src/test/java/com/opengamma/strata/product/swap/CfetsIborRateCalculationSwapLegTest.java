...
public class CfetsIborRateCalculationSwapLegTest {
  ...
  private static final LocalDate startDate = LocalDate.of(2024, Month.JULY, 3),
      ...
      tradeDate = LocalDate.of(2024, Month.JULY, 2);
  ...
  @Test
  public void testFR007Trade() {
    HolidayCalendar cnbe = ImmutableHolidayCalendar.of(CNBE_CALENDAR, holidays,
        Arrays.asList(DayOfWeek.SATURDAY, DayOfWeek.SUNDAY), workingWeekends);
    ReferenceData refData = ReferenceData
                                .of(ImmutableMap.<HolidayCalendarId, HolidayCalendar>builder().put(cnbe.getId(), cnbe).build());
    SwapTrade trade =
        FixedIborSwapConventions.CNY_REPO_1W_3M_A365F.createTrade(tradeDate, Tenor.TENOR_9M, BuySell.BUY, 400000000d,
            0.0195, refData);

    ResolvedSwapTrade resolve = trade.resolve(refData);
    ResolvedSwapLeg payLeg = resolve.getProduct().getPayLeg().orElse(null);
    ResolvedSwapLeg receiveLeg = resolve.getProduct().getReceiveLeg().orElse(null);
    ImmutableList<SwapPaymentPeriod> payLegPaymentPeriods = payLeg.getPaymentPeriods();
    ImmutableList<SwapPaymentPeriod> receivePaymentPeriods = receiveLeg.getPaymentPeriods();
  }
}
